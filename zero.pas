			...
			begin 
				if j=1 then //colonne gauche
						...
						if i=1 then // case haut gauche
							begin
								if ((grille[i,j+1].nbMinesAutour = 0) and not(grille[i,j+1].estAffiche)) then
									k := 1;
								if ((grille[i+1,j].nbMinesAutour = 0) and not(grille[i+1,j].estAffiche)) then //notifie si on a un zero autour de la case qui n'est pas dévoilé
									k := 1;
								if ((grille[i+1,j+1].nbMinesAutour = 0) and not(grille[i+1,j+1].estAffiche)) then
									k := 1;
								grille[i,j+1].estAffiche:= True; //dévoile les cases autour 
							...
							begin
								...
								if ((grille[i,j+1].nbMinesAutour = 0) and not(grille[i,j+1].estAffiche)) then
									k := 1;
							    grille[i-1,j].estAffiche:= True; //dévoile les cases autour 
							    grille[i-1,j+1].estAffiche:= True;
					...
					begin							
						...
						grille[i-1,j-1].estAffiche:= True; //dévoile les cases autour 
						grille[i,j-1].estAffiche:= True;
						grille[i+1,j-1].estAffiche:= True;
						grille[i-1,j].estAffiche:= True;
						grille[i+1,j].estAffiche:= True;
