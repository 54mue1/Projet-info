...
begin
    ...
    case touche of
        ...
        HAUT:  if curs1.y < MAX then curs1.y := curs1.y + 1;// déplacement haut
        BAS: if curs1.y > 1 then curs1.y := curs1.y - 1;  // déplacement bas
        GAUCHE: if curs1.x > 1 then curs1.x := curs1.x - 1;  // déplacement gauche
        DROITE: if curs1.x < MAX then curs1.x := curs1.x + 1;  // déplacement droite
        ENTREE: if not(grille1[curs1.y][curs1.x].estAffiche) then grille1[curs1.y][curs1.x].estAffiche:=True; // rends la case sur laquelle est le curseur dévoilable
        INSER :  if not(grille1[curs1.y][curs1.x].drapeau)   then grille1[curs1.y][curs1.x].drapeau:=True;  // pose un drapeau sur la case du curseur
